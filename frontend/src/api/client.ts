import type { AISessionStatus, AgentTurnRequest, AgentTurnResponse, ApiErrorBody, AreaStoresResponse, FinancePlanRequest, FinancePlanResponse, LeasePlanRequest, LeasePlanResponse, MarketGeographyRequest, MarketGeographyResponse, MetadataResponse, RecommendationRequest, RecommendationResponse, WebResearchRequest, WebResearchResponse, WorkspaceAgentTurnRequest, WorkspaceAgentTurnResponse } from '../types/api'
...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) {
    ...
    if (response.status === 401 && body.error?.code === 'AI_AUTH_REQUIRED') {
      window.dispatchEvent(new Event('locationfit:ai-session-expired'))
    }
...
export const api = {
  aiSessionStatus: () => request<AISessionStatus>('/ai/session'),
  createAiSession: (accessCode: string) =>
    request<AISessionStatus>('/ai/session', {
      method: 'POST',
