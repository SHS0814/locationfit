...
예비 창업자는 자연어로 업종·지역·고객 조건을 설명할 수 있다. OpenAI 기반 단일 상담 에이전트가 조건을 구조화하고 사용자의 명시적 확인을 받은 뒤 기존 결정론적 추천 엔진을 도구로 호출한다. AI는 점수와 매출을 생성하지 않으며 추천 후 현재 후보 간 비교만 수행한다.

## 웹서비스 구조

웹서비스는 `frontend/`와 `backend/`를 명시적으로 분리한다.

- `frontend/`: React, Vite, TypeScript 기반 추천 조건 폼·결과 목록·지도
