...
상담 중 서울 열린데이터나 한국부동산원 API를 실시간 호출하지 않으며 배포 아티팩트만 읽는다. 데이터베이스는 사용하지 않는다. 월 환산임대료 한도와 임대면적·상가 유형·층을 모두 입력하면 기존 종합점수 80%와 임대예산 적합도 20%를 결합해 재정렬한다. 총 창업예산만 입력하거나 예산을 입력하지 않으면 기존 순위와 점수를 유지한다.
