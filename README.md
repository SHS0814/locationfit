...
상담 중 서울 열린데이터나 한국부동산원 API를 실시간 호출하지 않으며 배포 아티팩트만 읽는다. 데이터베이스는 사용하지 않는다. 추천 지도는 서울시 `상권분석서비스(영역-상권)` SHP(EPSG:5181)를 WGS84로 변환한 실제 Polygon/MultiPolygon 경계를 표시한다. 월 환산임대료 한도와 임대면적·상가 유형·층을 모두 입력하면 기존 종합점수 80%와 임대예산 적합도 20%를 결합해 재정렬한다. 총 창업예산만 입력하거나 예산을 입력하지 않으면 기존 순위와 점수를 유지한다.

서비스 아티팩트를 다시 만들 때는 서울 열린데이터광장 OA-15560의 `서울시 상권분석서비스(영역-상권).zip`을 풀어 SHP 구성 파일을 `data/raw/area/`에 둔다. 빌드 결과인 `area_boundaries.parquet`에는 1,650개 상권코드별 GeoJSON 경계가 저장된다.

### 한국부동산원 상가 임대료 수동 갱신

한국부동산원 R-ONE의 상업용부동산 임대동향조사에서 소규모·중대형·집합 상가의 지역/층별 환산임대료와 전환율을 분기 단위로 내려받는다. 공식 표본상권 대표점과 서울 상권 중심점의 최단거리를 계산해 `area_code × 상가 유형` 정적 매핑을 만든다.

```bash
export REB_API_KEY='발급받은-인증키'
PYTHONPATH=. .venv/bin/python scripts/refresh_reb_commercial_rent.py
```
